/*Aquí empieza el script de login y registro*/

function validar_registro(){
    /*Almacenamos valores de las cajas en los que vamos a introducir 
    datos*/
    var mail, password, expresion;
    mail = document.getElementById("mail").value;
    password = document.getElementById("password").value;
    /*Estructura que tiene que tener un correo electrónico:
    estebanmorillo@icloud.com, .net, .es*/
    expresion = /\w+@\w+\.+[a-z]/;
    /*expresion permite definir que carácteres permitirás o en este caso todos los carácteres*/
    /**************************************************************************************/
    if(mail=== "" || password=== "" ){
        alert("Error! No ha introducido datos. Está vacío.Introduzca un password o mail para seguir");
        return false;
    } else if(mail.length > 25) {
        alert("Error! El número de carácteres no es valido. Supera el máximo permitido. Por favor, introduzca un mail válido");
        return false;
    } else if (!expresion.test(mail)){
        alert("Error! Mail no válido. Por favor introduzca un mail para seguir");
        return false;
    } else if (password.length <8 || password.length >20){
        alert("Error! Password no válido. Introduzca uno de 8 a 20 carácteres.");
        return false;
    } 
}
...
/*Aquí empieza el script de home*/
...
function slider(slider) {
	...
	// Controles
		...
		// Se añade la clase active para empezar en un elemento aleatorio del slider.
